...
template <typename T>
T acospi(T i) {
  return std::acos(i) / M_PI;
}
template <typename T>
T asinpi(T i) {
  return std::asin(i) / M_PI;
}
template <typename T>
T atanpi(T i) {
  return std::atan(i) / M_PI;
}
template <typename T>
T atan2pi(T i, T j) {
  return std::atan2(i, j) / M_PI;
}
template <typename T>
T cospi(T i) {
  return std::cos(M_PI * i);
}
template <typename T>
T divide(T i, T j) {
  return i / j;
